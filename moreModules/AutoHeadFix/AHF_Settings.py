...
class AHF_Settings (object):
    ...
    def __init__(self, fileName):
        ...
        if fileName is not None:
            ...
            # look for experiment config files in the current directory, they
            # start with AHFexp_ and end with .jsn
            iFile = 0
            files = ''
            for f in os.listdir('.'):
            ...
            if iFile == 0:  # no files found, create one
                print (
                    'Unable to find an experiment config file, let\'s make a new configuration:')
                fileNum = -1
            else:
            ...
            if fileNum == -1:
                ...
                # each list item starts with fileNum: so split the list item on
                # ":" and get item 1 to get file name
    ...
    def config_from_user(self):
        ...
        """
        self.fileName = ''
        # solenoid opening times, for entry rewards and task rewards, and other
        # reward related settings
        self.entranceRewardTime = float(
            input('Solenoid opening duration, in seconds, for entrance rewards:'))
        self.taskRewardTime = float(
            input('Solenoid opening duration,in seconds, for task rewards:'))
        self.maxEntryRewards = int(
            input('Maximum number of entry rewards that will be given per day:'))
        self.entryRewardDelay = float(
            input('Delay, in seconds, before an entrance reward is given:'))
        # head fix related settings
        self.propHeadFix = float(
            input('proportion (0 to 1) of trials that are head-fixed:'))
        # make sure proportion is bewteen 0 and 1
        self.propHeadFix = min(1, max(0, self.propHeadFix))
        self.skeddadleTime = float(input(
            'Time, in seconds, for mouse to get head off the contacts when session ends:'))
        tempInput = input(
            'Send text messages if mouse exceeds criterion time in chamber?(Y or N):')
        self.hasTextMsg = bool(tempInput[0] == 'y' or tempInput[0] == 'Y')
        if self.hasTextMsg == True:
            self.inChamberTimeLimit = float(input(
                'In-Chamber duration limit, seconds, before stopping head-fix trials and sending text messg:'))
            self.phoneList = tuple(input(
                'Phone numbers to receive a text message if mouse is in chamber too long:').split(','))
        else:
            self.inChamberTimeLimit = float(
                input('In-Chamber duration limit, seconds, before stopping head-fix trials:'))
       # Camera related settings, in a dictionary, static function, don't need
       # a camera object to be created
        self.camParamsDict = AHF_Camera.dict_from_user({})
        # UDP stuff - make a tuple of IP address of other computers
        tempInput = input(
            'Send UDP triggers to start secondary cameras (Y or N):')
        self.hasUDP = bool(tempInput[0] == 'y' or tempInput[0] == 'Y')
        if self.hasUDP == True:
            self.UDPList = tuple(
                input('IP addresses of Pis running secondary cameras:').split(','))
            self.cameraStartDelay = float(
                input('Delay in seconds between sending UDP and toggling blue LED.'))
        # Stimulator class
        self.stimulator = AHF_Stimulator.get_stimulator_from_user()
        # static function to make a configration without needing a stimulator
        # to configure it
        self.stimDict = AHF_Stimulator.get_class(
            self.stimulator).dict_from_user({})
    ...
    def save(self):
        ...
        if self.fileName != '':
            newConfig = input(
                'Enter a name to save config, or enter to use current name, \'' + self.fileName + '\':')
            if newConfig == '':
        ...
        if newConfig != self.fileName:
            newConfig = 'AFHexp_' + \
                ''.join([c for c in newConfig if c.isalpha()
                         or c.isdigit() or c == '_']) + '.jsn'
        ...
        with open(newConfig, 'w') as fp:
            ...
            # we run AutoHeadFix as root for GPIO, so expicitly set ownership
            # for easy editing
            os.chown(newConfig, uid, gid)
    ...
    def load(self, file):
        with open(file, 'r') as fp:
            ...
            if self.hasUDP == True:
                ...
                self.cameraStartDelay = float(
                    configDict.get('cameraStartDelay'))
    ...
    def show(self):
        ...
        print ('1:Entrance Reward Time (secs) =' +
               str(self.entranceRewardTime))
        ...
        print ('5:Proportion of Contacts to Head Fix (0-1) =' +
               str(self.propHeadFix))
        print ('6:Time for Mouse to break contact after trial (secs) =' +
               str(self.skeddadleTime))
        print ('7:Use Text Messaging =' + str(self.hasTextMsg))
        if self.hasTextMsg == True:
            print ('\t7_a:List of Phone Numbers=' + str(self.phoneList))
            print ('8:Duration in chamber (secs) before stopping trials and sending message =' +
                   str(self.inChamberTimeLimit))
        else:
            print ('8:Duration in chamber (secs) before stopping trials =' +
                   str(self.inChamberTimeLimit))
        print ('9:Send UDP triggers = ' + str(self.hasUDP))
        if self.hasUDP == True:
            print ('\t9_a:List of ip addresses for UDP = ' + str(self.UDPList))
            print ('\t9_b:Camera start to LED ON delay (secs) =' +
                   str(self.cameraStartDelay))
        print ('10:Stimulator = ' + self.stimulator)
        i = 0
        for key in sorted(self.stimDict.keys()):
            print ('\t10_' + chr(97 + i) + ": " +
                   key + ' = ' + str(self.stimDict[key]))
            i += 1
    ...
    def edit_from_user(self):
        ...
        while True:
            self.show()
            editNum = input(
                'Enter number of paramater to Edit, or 0 when done:')
            if editNum == '0':
                ...
                tempInput = input(
                    'Send text messages if mouse exceeds criterion time in chamber?(Y or N):')
                self.hasTextMsg = bool(
                    tempInput[0] == 'y' or tempInput[0] == 'Y')
                ...
                self.inChamberTimeLimit = float(input(
                    'Time limit in seconds for mouse being in the chamber before stopping headfixes, optionally sending text mssg:'))
            elif editNum == '9':
                tempInput = input(
                    'Send UDP triggers to start secondary cameras (Y or N):')
                self.hasUDP = bool(tempInput[0] == 'y' or tempInput[0] == 'Y')
                ...
                for key in sorted(self.stimDict.keys()):
                    if i == selectedKey:
                        newValue = input(
                            'Set ' + key + ' (currently ' + str(self.stimDict.get(key)) + ') to:')
