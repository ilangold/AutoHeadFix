...
class AHF_Stimulator (object):
    ...
    def configStim(self, mouse):
        ...
        if 'stimCount' in mouse.stimResultsDict:
            mouse.stimResultsDict.update(
                {'stimCount': mouse.stimResultsDict.get('stimCount') + 1})
        else:
    ...
    def get_stimulator_from_user():
        ...
        if iFile == 0:
            print (
                'Could not find an AHF_Stimulator_ file in the current or enclosing directory')
            ...
            if iFile == 1:
                ...
                inputStr = '\nEnter a number from 0 to ' + \
                    str(iFile - 1) + ' to Choose a Stimulator class:\n'
                ii = 0
    ...
    def dict_from_user(stimDict):
        ...
        if stimDict is not None and len(stimDict.keys()) > 0:
            for key in sorted(stimDict.keys()):
                tempInput = input(
                    'set ' + key + '(currently ' + str(stimDict[key]) + ') to :')
                if tempInput != '':
