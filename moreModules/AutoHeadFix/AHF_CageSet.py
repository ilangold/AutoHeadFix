...
"""
    ...
    def __init__(self):
        ...
        try:
            ...
            print (
                'Unable to open base confiuration file, AHFconfig.jsn, let\'s make a new one.\n')
            self.cageID = input('Enter the cage ID:')
            self.pistonsPin = int(
                input('Enter the GPIO pin connected to the Head Fixing pistons:'))
            self.rewardPin = int(
                input('Enter the GPIO pin connected to the water delivery solenoid:'))
            self.tirPin = int(
                input('Enter the GPIO pin connected to the Tag-in-Range pin on the Tag reader:'))
            self.contactPin = int(
                input('Enter the GPIO pin connected to the headbar contacts:'))
            self.ledPin = int(
                input('Enter the GPIO pin connected to the blue LED for camera illumination:'))
            self.serialPort = input(
                'Enter serial port for tag reader(likely either /dev/ttyAMA0 or /dev/ttyUSB0):')
            self.dataPath = input(
                'Enter the path to the directory where the data will be saved:')
            self.show()
            doSave = input(
                'Enter \'e\' to re-edit the new Cage settings, or any other character to save the new settings to a file.')
    ...
    def save(self):
        ...
        Call this function after modifying the contents of the AHF_CageSet to save your changes
           ...
           :returns: nothing
        """
        jsonDict = {'Cage ID': self.cageID,
                    'Pistons Pin': self.pistonsPin, 'Reward Pin': self.rewardPin}
        jsonDict.update({'Tag In Range Pin': self.tirPin,
                         'Head Contact Pin':  self.contactPin, 'LED Pin': self.ledPin})
        jsonDict.update({'Serial Port': self.serialPort,
                         'Path to Save Data': self.dataPath})
        with open('AHFconfig.jsn', 'w') as fp:
            fp.write(json.dumps(jsonDict))
            fp.close()
            uid = pwd.getpwnam('pi').pw_uid
            gid = grp.getgrnam('pi').gr_gid
            # we run as root for GPIO, so we need to expicitly set ownership
            os.chown('AHFconfig.jsn', uid, gid)
    ...
    def edit(self):
        ...
        while True:
            self.show()
            editNum = int(
                input('Enter number of paramater to Edit, or 0 when done to save file:'))
            if editNum == 0:
                break
            elif editNum == 1:
                self.cageID = input('Enter the cage ID:')
...
if __name__ == '__main__':
    ...
    print ('Cage ID:', hardWare.cageID,
           '\tPistons Pin:', hardWare.pistonsPin, '\n')
    hardWare.edit()
    hardWare.save()
