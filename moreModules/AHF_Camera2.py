...
def showConfig(configDict, camera2):
    ...
    print ('10:File path to the folder where the recorded video will be stored: ' +
           configDict.get('dataPath'))
    print ('11:IP address of the Rpi running the main task, which sends the start and stop signals: ' +
           configDict.get('UDP_Sender'))
    print ('12:Port number to use for UDP - any  non-assigned Port number will do: ' +
           str(configDict.get('UDP_Port')))
    print ('13:IP address of the interface on this computer used to listen for UDP signals, or \'\' to listen on all interfaces: ' +
           str(configDict.get('UDP_IP')))
    print ('14:Maximum number of seconds to record video after getting a start signal, ' +
           str(configDict.get('maxRecSecs')))
...
def editConfig(configDict, camera2):
    ...
    while True:
        ...
        selNum = int(input(
            'Enter a number to adjust settings, or 0 to save the settings and start waiting for events:'))
        if selNum == 0:
        ...
        if selNum < 10:
            ...
            # add camera settings to config dict so they can be saved to file
            configDict.update(camera2.get_configDict())
        elif selNum == 10:
            tempInput = input('Enter file path:')
            configDict.update({'dataPath': tempInput})
        elif selNum == 11:
            tempInput = input(
                'Enter IP address of the Rpi running the main task:')
            ...
            tempInput = input(
                'Enter maximum number of seconds to record video:')
